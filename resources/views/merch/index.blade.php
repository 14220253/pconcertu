...
<body>
    ...
    <div class="bg-white">
        <div class="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
            <h2 class="text-3xl font-bold tracking-tight text-gray-900 py-3 my-3">MERCH</h2>

            <div class="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
                @foreach ($merches as $merch)
                    
                    <a href="{{ route('merch.show', $merch) }}" class="group" >
                        <div
                            class="aspect-h-1 aspect-w-1 w-full overflow-hidden rounded-lg bg-gray-200 xl:aspect-h-8 xl:aspect-w-7">
                        </div>
                        <h3 class="mt-4 text-sm text-gray-700">{{ $merch->size }} - {{ $merch->color }} - {{ $merch->variation }} - {{ $merch->stock }}</h3>
                        <p class="mt-1 text-lg font-medium text-gray-900">event: {{ $merch->event_id }}</p>
                    </a>
                @endforeach
